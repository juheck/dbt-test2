...
# Parse nodes
import json
JSON_MANIFEST_DBT = '/home/airflow/gcs/data/dbt/airflow/manifest.json' # env variables
...
for node in nodes:
    ...
    bsh_cmd = 'dbt run --profiles-dir {DBT_PROJECT_DIR} --project-dir {DBT_PROJECT_DIR} --models {nodeName} --vars \'{{"start_date":"{start_date}", "end_date":"{end_date}"}}\' '.format(DBT_PROJECT_DIR = DBT_PROJECT_DIR, nodeName = node, start_date = date_start, end_date = date_end)
